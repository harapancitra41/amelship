...
enum PowerUpTypes { health, freeze, nuke, multiFire, laserWeapon, plasmaWeapon, fireWeapon }

// This class/component is responsible for spawning random power ups
// at random locations in the game world.
class PowerUpManager extends Component with HasGameReference<AmelShipGame> {
  ...
  static late Sprite laserWeaponSprite;
  static late Sprite plasmaWeaponSprite;
  static late Sprite fireWeaponSprite;
  ...
  static final PowerUpMap _powerUpMap = {
    ...
    PowerUpTypes.laserWeapon: (position, size) =>
        LaserWeapon(position: position, size: size),
    PowerUpTypes.plasmaWeapon: (position, size) =>
        PlasmaWeapon(position: position, size: size),
    PowerUpTypes.fireWeapon: (position, size) =>
        FireWeapon(position: position, size: size),
  };
  ...
  void onMount() {
    ...
    // Use existing sprites as placeholders for weapon power-ups
    // (they will be colored via particle effects when activated)
    laserWeaponSprite = Sprite(game.images.fromCache('nuke.png'));
    plasmaWeaponSprite = Sprite(game.images.fromCache('freeze.png'));
    fireWeaponSprite = Sprite(game.images.fromCache('multi_fire.png'));

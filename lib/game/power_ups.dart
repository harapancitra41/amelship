...
abstract class PowerUp extends SpriteComponent
    with HasGameReference<AmelShipGame>, CollisionCallbacks {
...
// Weapon power-up: Laser (red) - faster, double damage
class LaserWeapon extends PowerUp {
  LaserWeapon({super.position, super.size});

  @override
  Sprite getSprite() {
    return PowerUpManager.laserWeaponSprite;
  }

  @override
  void onActivated() {
    // Register a command to set weapon to laser (type 1).
    final command = Command<Player>(
      action: (player) {
        player.setWeapon(1);
      },
...
// Weapon power-up: Plasma (cyan) - larger, penetrates
class PlasmaWeapon extends PowerUp {
  PlasmaWeapon({super.position, super.size});

  @override
  Sprite getSprite() {
    return PowerUpManager.plasmaWeaponSprite;
  }

  @override
  void onActivated() {
    // Register a command to set weapon to plasma (type 2).
    final command = Command<Player>(
      action: (player) {
        player.setWeapon(2);
      },
...
// Weapon power-up: Fire (orange) - triple damage
class FireWeapon extends PowerUp {
  FireWeapon({super.position, super.size});

  @override
  Sprite getSprite() {
    return PowerUpManager.fireWeaponSprite;
  }

  @override
  void onActivated() {
    // Register a command to set weapon to fire (type 3).
    final command = Command<Player>(
      action: (player) {
        player.setWeapon(3);
      },

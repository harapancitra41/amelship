...
class Player extends SpriteComponent
    with CollisionCallbacks, HasGameReference<AmelShipGame>, KeyboardHandler {
  // Player joystick
  ...
  // Weapon types: 0 = normal, 1 = laser (red), 2 = plasma (cyan), 3 = fire (orange)
  int _currentWeapon = 0;
  int get currentWeapon => _currentWeapon;
  ...
  // Controls for how long weapon power up is active.
  ...
  Player({
    ...
    // Sets weapon timer to 5 seconds. After 5 seconds,
    // weapon will reset to normal.
    _weaponTimer = Timer(
      5,
      onTick: () {
        _currentWeapon = 0;
  ...
  void update(double dt) {
    ...
    // Colorful thruster particles based on weapon type.
    ...
    final particleColor = particleColors[_currentWeapon % particleColors.length];
  ...
  void joystickAction() {
    // Create bullet with weapon type
  ...
  // Sets weapon type for player. Weapon types:
  // 0 = normal (white)
  // 1 = laser (red) - faster, more damage
  // 2 = plasma (cyan) - larger, penetrates
  // 3 = fire (orange) - splash damage

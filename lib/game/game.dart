...
class AmelShipGame extends FlameGame
  ...
  // Displays current weapon type.
  late TextComponent _currentWeapon;
  ...
  Future<void> onLoad() async {
    ...
    if (!_isAlreadyLoaded) {
      ...
      // Create text component for current weapon.
      _currentWeapon = TextComponent(
        text: 'Senjata: Normal',
  ...
  void update(double dt) {
    ...
    if (isAttached && _player.isReady) {
      ...
      _playerScore.text = 'Skor: ${_player.score}';
      _playerHealth.text = 'Kesehatan: ${_player.health}%';

      // Update weapon display based on current weapon type
      final weaponNames = ['Normal', 'Laser', 'Plasma', 'Api'];
      final weaponColors = [Colors.white, Colors.red, Colors.cyan, Colors.orange];
      final weaponIndex = _player.currentWeapon.clamp(0, 3);
      _currentWeapon.text = 'Senjata: ${weaponNames[weaponIndex]}';
      _currentWeapon.textRenderer = TextPaint(
        style: TextStyle(
          color: weaponColors[weaponIndex],
          fontSize: 12,
          fontFamily: 'BungeeInline',
        ),
      );
